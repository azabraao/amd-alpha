<?php
	...
	class UpdateUsuario{
		...
		function selecionaNomeUsuario($id_usuario){
			require_once('conexao.php');
			$objBd = new bd();
			$link  = $objBd->conecta_mysql(); 

			$sql = " SELECT nome_usuario FROM tb_usuarios WHERE id_usuario = $id_usuario ";

			$retorno_bd = mysqli_query($link,$sql);

			if($retorno_bd){
				while($dados_bd	= mysqli_fetch_array($retorno_bd)){
					$nome = $dados_bd['nome_usuario'];
				}
		...
		function selecionaSobrenomeNomeUsuario($id_usuario){
			require_once('conexao.php');
			$objBd = new bd();
			$link  = $objBd->conecta_mysql(); 

			$sql = " SELECT sobrenome_usuario FROM tb_usuarios WHERE id_usuario = $id_usuario ";

			$retorno_bd = mysqli_query($link,$sql);

			if($retorno_bd){
				while($dados_bd	= mysqli_fetch_array($retorno_bd)){
					$sobrenome = $dados_bd['sobrenome_usuario'];
				}
		...
		function selecionaHierarquia($id_usuario){
			require_once('conexao.php');
			$objBd = new bd();
			$link  = $objBd->conecta_mysql(); 
			...
			if($retorno_bd){
				while($dados_bd	= mysqli_fetch_array($retorno_bd)){
					$hierarquia = $dados_bd['hierarquia'];
				}
		...
		function selecionaEmail($id_usuario,$email){
			//testa pra ver se existe 	
			...
			if($testeEmail['email'] == ''){
				...
				echo "Email já cadastrado para outro usuário.";
			}
		...
		function selecionaTipoObra(){
			...
			$sql = " SELECT tipo_obra FROM tb_obras WHERE id_usuario = $id_usuario ";
		...
		function selecionaContratante($id_usuario){
		...
		function selecionaObras(){
			...
			$id_usuario = $_SESSION['id_usuario']; 
				...
				$sql ="	SELECT	id_usuario,nome_usuario,tipo_obra,id_contratante,fim_obra,inicio_obra, DATEDIFF(fim_obra,inicio_obra) as qtd_dias FROM tb_obras WHERE istatus_obra = 0 ";
						...
						while($obras = mysqli_fetch_array($resultado_bd)){ 
				          ...
				          if($interval->format('%R%a days') < 0){
							...
							echo "<div class='obras-adm'>
                    					<a href='editar_obra.php?cont=$id_contratante&id_usuario=".$obras['id_usuario']."' style='color:#ccc'> 
                    						<span class='glyphicon glyphicon-edit opcao-editar'></span> 
                    					</a>
										<a href='acompanhamento.php?id_usuario=".$obras['id_usuario']."' style='color:white'>
	                      					<h3>".$obras['nome_usuario']."</h3><br>
		                    				<small> 
		...
		function selecionaCliente1(){
			...
			$sql = " SELECT 
					 ...
					 WHERE us.hierarquia = 1 and rel.acesso_id_usuario = $id_usuario ORDER BY us.id_usuario ASC";
		...
		function selecionaCliente2(){
			...
			$sql = " SELECT 
					 ...
					 FROM tb_relacao_usuario_obra rel
					 INNER JOIN tb_usuarios us on rel.id_usuario = us.id_usuario
					 WHERE us.hierarquia = 1 and rel.acesso_id_usuario = $id_usuario ORDER BY us.id_usuario DESC ";
		...
		function selecionaEnderecoObra(){
			...
			$sql = " SELECT endereco FROM tb_obras WHERE id_usuario = $id_usuario ";
		...
		function selecionaInicioFim(){
			...
			$sql = " SELECT inicio_obra, fim_obra FROM tb_obras WHERE id_usuario = $id_usuario ";
		...
		function selecionaInicio(){
			...
			$sql = " SELECT inicio_obra FROM tb_obras WHERE id_usuario = $id_usuario ";
		...
		function selecionaFim(){
			...
			$sql = " SELECT fim_obra FROM tb_obras WHERE id_usuario = $id_usuario ";
		...
		function selecionaEncarregado(){
			...
			$sql = " SELECT 
					 ...
					 FROM tb_relacao_usuario_obra rel
					 INNER JOIN tb_usuarios us on rel.id_usuario = us.id_usuario
					 WHERE us.hierarquia = 2 and rel.acesso_id_usuario = $id_usuario ";
		...
		function selecionaObrasConcluidas(){
			...
			$id_usuario = $_SESSION['id_usuario']; 
				...
				$sql ="	SELECT	id_usuario,nome_usuario,tipo_obra,id_contratante,fim_obra,inicio_obra, DATEDIFF(fim_obra,inicio_obra) as qtd_dias FROM tb_obras WHERE istatus_obra = 2 ";
           ...
           <form action='classes/restaura_item.php' method='POST'>
                ...
                <div class='form-group' style='display: inline-block;'>
                  <input type='hidden' name='id_usuario' value=\"{$obras['id_usuario']}\">
		...
		function selecionaObrasArquivadas(){
			...
			$id_usuario = $_SESSION['id_usuario']; 
				...
				$sql ="	SELECT	id_usuario,nome_usuario,tipo_obra,id_contratante,fim_obra,inicio_obra, DATEDIFF(fim_obra,inicio_obra) as qtd_dias FROM tb_obras WHERE istatus_obra = 1 ";
